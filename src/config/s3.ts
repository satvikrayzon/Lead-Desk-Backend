...
function getS3Client(): S3Client {
  ...
  if (!s3Client) {
    const usingCustomEndpoint = !!env.S3_ENDPOINT;
    ...
    s3Client = new S3Client({
      ...
      // DigitalOcean Spaces / Ceph reject AWS flexible checksum headers.
      ...(usingCustomEndpoint
        ? {
...
export async function uploadToS3(params: {
  ...
  // SSE-AES256 is AWS-only; DigitalOcean Spaces returns InvalidArgument if sent.
  const putInput: {
    Bucket: string;
    Key: string;
    Body: Buffer;
